...
public class SlidingWindowAnalysisService {
    ...
    public SlidingWindowAnalysisService(
            StockDataIngestionService ingestionService,
    ...
    public void setupAnalysis() {
        ...
        ingestionService.getTradeStream()
    ...
    private Mono<Void> processTrade(MarketTrade trade) {
        ...
        return redisTemplate.opsForZSet().add(key, String.valueOf(trade.price()) + ":" + now, now)
                ...
                .thenMany(redisTemplate.opsForZSet().range(key, Range.unbounded()))
                .collectList()
                .flatMap(items -> analyzeWindow(trade.symbol(), items, trade.price()))
                .then();
    }
    ...
    private Mono<Void> triggerAlert(String symbol, double minPrice, double maxPrice, double percentChange,
        ...
        return redisTemplate.opsForValue().setIfAbsent(deduplicationKey, "locked", lockDuration)
                .flatMap(locked -> {
                    if (Boolean.TRUE.equals(locked)) {
                        String message = String.format("🚨 <b>Market Alert</b> 🚨\n\n" +
                                "Symbol: <b>%s</b>\n" +
                                "Direction: <b>%s</b>\n" +
                                "Change: %.2f%%\n" +
                                "Min: $%.2f\n" +
